...
import { runCommands, ensureReleasedAPILinks } from './preprocessor'
import { lint as checkPublicAPI } from './check_public_api'
...
async function run() {
	...
	// Documentation checks.
	{
		...
		// )
		const mdSources = [readme, api, contributing]

		messages.push(...(await runCommands(mdSources, VERSION)))
		messages.push(...(await ensureReleasedAPILinks([readme], VERSION)))

		const browser = await puppeteer.launch()
		const page = await browser.newPage()
		const jsSources = await Source.readdir(path.join(PROJECT_DIR, 'packages/core/src'), '.ts')
	...
	if (changedFiles) {
		if (clearExit) console.log(`${YELLOW_COLOR}Some files were updated.${RESET_COLOR}`)
		clearExit = false
	}
