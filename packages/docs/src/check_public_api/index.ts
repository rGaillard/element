...
import { Documentation, Class } from './Documentation'
 ...
 */
export async function lint(page, mdSources, jsSources) {
 ...
 */
function filterJSDocumentation(jsSources: Source[], jsDocumentation) {
...
function checkDuplicates(doc) {
	...
	// Report duplicates.
	for (const cls of doc.classesArray) {
		if (classes.has(cls.name)) errors.push(`Duplicate declaration of class ${cls.name}`)
...
function compareDocumentations(actual, expected) {
	...
	for (const className of classesDiff.extra) errors.push(`Non-existing class found: ${className}`)
	for (const className of classesDiff.missing) errors.push(`Class not found: ${className}`)
	...
	for (const className of classesDiff.equal) {
		...
		for (const methodName of methodDiff.equal) {
			...
			if (!actualMethod.type !== !expectedMethod.type) {
				...
				else errors.push(`Method ${className}.${methodName} is missing return type description`)
			...
			if (argsDiff.extra.length || argsDiff.missing.length) {
				...
				for (const arg of argsDiff.missing) text.push(`- Argument not found: ${arg}`)
				for (const arg of argsDiff.extra) text.push(`- Non-existing argument found: ${arg}`)
	...
	function checkType(source, actual, expected) {
		...
		if (actual.name.includes('unction') || actual.name.includes('Serializable')) return
		...
		if (expectedName !== actualName) errors.push(`${source} ${actualName} != ${expectedName}`)
