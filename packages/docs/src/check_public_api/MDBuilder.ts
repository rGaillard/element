...
import { Documentation, Member, Type, Class } from './Documentation'
...
class MDOutline {
	...
	static async create(page, text) {
		...
		const { classes, errors } = await page.evaluate(() => {
			 ...
			 */
			function parseProperty(element: HTMLLIElement): any {
				const clone = element.cloneNode(true)
				const ul = (clone as HTMLLIElement).querySelector(':scope > ul')
				...
				if (!type.includes('string')) {
					for (const childElement of Array.from(
						element.querySelectorAll<HTMLLIElement>(':scope > ul > li'),
					)) {
	...
	constructor(classes, errors) {
		...
		function handleMethod(member, className, methodName, parameters) {
			...
			const method = Member.createMethod(
				methodName,
				args,
				returnType,
				returnComment,
		...
		function createPropertyFromJSON(payload) {
			const type = new Type(payload.type, payload.properties.map(createPropertyFromJSON))
			const required = payload.required
			return Member.createProperty(payload.name, type, payload.comment, required)
		}
		...
		function handleProperty(member, className, propertyName) {
			...
			currentClassMembers.push(
				createPropertyFromJSON({ type, name: propertyName, properties, comment: member.comment }),
			)
		}
		...
		function flushClassIfNeeded() {
			...
			this.classes.push(
				new Class(currentClassName, currentClassMembers, currentClassExtends, currentClassComment),
			)
 ...
 */
export default async function(page, sources) {
	const classes: any[] = []
	const errors: any[] = []
