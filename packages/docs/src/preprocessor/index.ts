...
export const ensureReleasedAPILinks = function(sources, version) {
...
export const runCommands = function(sources, version) {
	// Release version is everything that doesn't include "-".
	...
	for (const command of commands) {
		...
		if (command.name === 'version') newText = isReleaseVersion ? 'v' + version : 'Tip-Of-Tree'
		...
		if (newText === null) messages.push(Message.error(`Unknown command 'gen:${command.name}'`))
