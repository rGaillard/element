...
import MDBuilder from '../check_public_api/MDBuilder'
...
${root.methodsArray
	.map(
		method => `
${memberJSDOC(method, '')}export function ${method.name}${argsFromMember(method)} : ${typeToString(
			method.type,
			method.name,
		)};
`,
	...
	while ((definition = objectDefinitions.pop())) {
		...
		parts.push(
			properties
				.map(
					member =>
						`  ${memberJSDOC(member, '  ')}${nameForProperty(member)}${argsFromMember(
							member,
							name,
						)}: ${typeToString(member.type, name, member.name)};`,
				)
				.join('\n\n'),
		)
		parts.push('}\n')
...
function parseType(type: string | null) {
	...
	if (type.startsWith('...')) return parseType('Array<' + type.substring(3) + '>')
	let name = type
	let next = null
	let template = null
	let args = null
	...
	let pipe = null
	if (type[firstTypeLength] === '|') pipe = parseType(type.substring(firstTypeLength + 1))
	else if (type[firstTypeLength] === ',') next = parseType(type.substring(firstTypeLength + 1))
...
function stringifyType(parsedType) {
	...
	if (parsedType.template) out += '<' + stringifyType(parsedType.template) + '>'
...
function memberJSDOC(member, indent) {
	...
	lines.push(
		...member.argsArray.map(
			arg => `@param ${arg.name.replace(/\./g, '')} ${arg.comment.replace('\n', ' ')}`,
		),
	)
